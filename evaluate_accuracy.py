...
def evaluate_with_iou(pred_csv_path, gt_csv_path, iou_threshold=0.5, output_csv=None):
    ...
    #all_frames = sorted(set(df_gt["frame"]).union(set(df_pred["frame"]))) #all frames of both, creates false negatives in frames without detection.
    #all_frames = sorted(set(df_gt["frame"]).intersection(set(df_pred["frame"]))) #compares only frames from both CSVs → for which there was a prediction and a ground truth.
    
    # 🧠 Consider only frames with at least one GT with a valid 'true'
    ...
    print(f"✅ Total Accuracy (IoU ≥ {iou_threshold}): {total_accuracy:.4f}")
    print(f"📈 Precision: {precision:.4f}")
    print(f"📈 Recall:   {recall:.4f}")
    print(f"📈 F1-Score: {f1:.4f}\n")
    print("📊 Accuracy per cow:")
    print(accuracy_per_class)

    if output_csv:
        result_df.to_csv(output_csv, index=False)
        print(f"📄 Detailed results saved at: {output_csv}")

    return total_accuracy, accuracy_per_class, precision, recall, f1

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Evals accuracy prioritizing IoU + name (normalized names)")
    ...
    parser.add_argument('--save', help="Path to save detailed CSV")
    parser.add_argument('--iou-thresh', type=float, default=0.5, help="Minimum IoU to consider a match")
